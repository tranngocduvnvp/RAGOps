...
def setup_pipeline(args):
    ...
    return semanticRouter, llm, reflection, rag, reranker, memorymodule
...
async def lifespan(app: FastAPI):
    ...
    print("Config Loaded")
    semanticRouter, llm, reflection, rag, reranker, memorymodule = setup_pipeline(args)
...
async def openai_compatible_chat(req: OpenAIChatRequest):
    ...
    try:
        ...
        stm = memorymodule.get_short_term_history("001")
        ltm = memorymodule.search("001", query=user_content)
        ...
        if guidedRoute == PRODUCT_ROUTE_NAME:
            ...
            memorymodule.ingress("001", prompt_messages + [{"role": "assistant", "content": assistant_response}])
        else:
